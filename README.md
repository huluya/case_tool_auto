...
本版本主要完善 Excel 分批导入、分页操作、编辑稳定性和自定义列使用体验。

### 更新内容

1. Excel 导入支持带命名空间的标准文件格式，修复部分文件被识别为 0 条用例的问题。
2. Excel 导入改为追加到当前版本底部，已有用例保持原顺序，编号按列表顺序自动整理。
3. 分页新增 200 条/页和 300 条/页，减少大批量用例合并时的跨页限制。
4. 修复单击直接编辑与双击大文本编辑之间的重复触发和误取消问题。
5. 修复图片一次粘贴被重复插入的问题；外部粘贴文字会自动清理网页样式和无用标签。
6. 修复批量删除全部用例后按钮仍显示旧数量的问题。
7. 添加自定义列时只需输入显示名称，内部字段标识由系统自动生成。
8. 继续保留 PyCharm 控制台日志和 `logs/case_manager.log` 文件日志输出。

## 1.0.2 发布说明

本版本围绕用例导入、版本隔离、合并单元格、富文本图片、权限和日常测试流程进行了整理和修复。

### 需求清单

1. Excel 导入按第一行自动识别列数；系统表头自动映射，其他表头自动生成自定义列；支持导入纵向合并、删除线和自动编号。
2. 用例编号可在列设置中按当前列表顺序重置为 1～n；合并的标题按逻辑用例计数，不影响真实用例行。
3. 项目和版本支持新建、编辑、删除、切换、版本排序、唯一命名和版本副本；项目/版本管理按钮受编辑模式和角色权限控制。
4. 角色和账号存储在数据库中，支持管理员、测试和只读角色；页面显示当前账号及权限提示，登录页不预填账号密码。
5. 编辑模式支持单击单元格直接输入、双击打开大文本编辑；编辑字段跟随当前可见列；执行结果不受编辑模式限制。
6. 列设置支持显示/隐藏、拖动排序、系统列一键整理、自定义列编辑/转换；列设置按版本隔离。
7. 支持同一列纵向合并和取消合并，合并内容水平/垂直居中；新增行时保持原有合并关系。
8. 备注、前置条件、操作步骤、预期结果和自定义列支持文字与图片混排；图片缩略图、删除、放大和拖动查看，图片本体存入 MySQL。
9. 支持关键词搜索和执行结果多选过滤；执行总结按合并后的逻辑用例统计。
10. 支持导出当前版本 Excel，保留可见列、顺序、列宽、删除线、执行结果颜色、合并关系和备注内嵌图片。
11. 操作列固定在表格最后，并支持收缩/展开；没有版本的项目只显示提示，不显示无意义的复选框和操作列。

### 缺陷清单

- 修复导入文件解析异常、损坏的 XLSX 提示不清和上传流兼容问题。
- 修复列设置按项目共用导致其他版本列配置和用例显示异常的问题，改为项目下的版本独立配置。
- 修复合并接口路径错误导致的 404，以及合并后只显示第一条内容的问题。
- 修复导入多行用例、合并用例、插入行时编号混乱、后续行错位和执行结果落入备注列的问题。
- 修复新增行时临时取消合并显示的问题；合并块内部插入会调整到合并块边界，避免截断 rowspan。
- 修复已执行结果颜色刷新不及时、总结按钮因物理行仍为“未执行”而无法点亮的问题。
- 修复富文本中的 `<span>`、`<div>` 等标签被当作普通文字显示，以及 Excel 多个 span 挤成一行的问题。
- 修复列靠左、居中、靠右被合并样式或富文本自身样式覆盖的问题。
- 修复图片粘贴只显示“[图片]”、编辑后图片消失和图片未随数据库保存的问题。
- 修复操作列按钮被挤压、标题与按钮错位，以及无版本项目显示复选框和操作列的问题。
- 修复自定义列导入后不能编辑、版本切换显示空数据、搜索过滤范围不完整等问题。

1.0.2 提交说明：完善需求与缺陷文档，修复上述用例管理、权限、合并、富文本、导入导出和界面显示问题。
...
角色和账号保存在 MySQL 的 `roles`、`users` 表中，不在代码中写死。只读角色只能查看和导出；测试角色可以编辑用例和执行结果；管理员可以管理项目、版本、列设置、用例和数据库备份。项目和版本删除需要删除密码，并且会弹出二次确认。账号密码和删除密码以实际部署配置为准。
...
导出当前版本时，按照当前可见列、列顺序和列宽生成 Excel，保留删除线、执行结果颜色、纵向合并以及备注中的图片。图片以 Excel 图片对象嵌入并锚定在对应备注单元格位置。

## 使用说明

下面按页面上的实际按钮说明各项操作。文中的截图来自真实页面，使用的是临时示例项目和演示数据；截图保留用于说明界面，临时数据已在截图完成后清理。页面按钮会根据当前账号和“编辑模式”开关显示或隐藏。
...
![项目与版本列表及版本右键菜单](docs/images/real-version-copy.png)
...
![编辑模式下的完整用例表](docs/images/real-case-table.png)
...
在顶部搜索框输入标题、用例编号或模块关键词，点击“搜索”即可筛选当前版本的用例。清空关键词后再次搜索可以恢复全部用例。

![搜索用例](docs/images/real-search.png)
...
- 单条删除位于编辑模式下的最右侧“操作”列。当前列较多时，用例表横向滚动到最右侧即可看到操作列，操作列不会挤压前面的列。操作列标题旁可以收缩/展开，收缩后只保留展开入口。
- 如果当前项目没有版本，列表只显示“该项目暂无版本，请先新建版本”，不会显示复选框和操作列。
...
![备注编辑窗口中的图片](docs/images/real-editor-image.png)
...
点击顶部“列设置”打开列管理窗口。每一行对应一个字段，左侧复选框控制是否显示，中间拖动标记控制顺序。列设置属于当前版本，不同版本之间互不影响。

![列设置窗口](docs/images/real-column-settings.png)

*示例数据截图：系统列完整展示，支持显示隐藏、拖动排序、重置编号和重置测试结果。*
...
- 点击“添加自定义列”，只需输入显示名称；内部字段标识由系统自动生成。
- 自定义列可以拖动、隐藏、显示和编辑。
...
6. 导入内容会追加到当前版本底部，已有用例保持原顺序；导入完成后按当前列表顺序自动整理编号，也可从列设置中重置为 1～n。
7. “执行结果/状态”会识别通过、失败、未执行、阻塞、跳过及常见英文写法，列表颜色会按状态显示。
8. 表格中单元格字体带删除线时，导入后会保留删除线。

![Excel 导入窗口](docs/images/real-import.png)

![导入后的用例列表和删除线示例](docs/images/real-case-table.png)
...
分页支持 20、50、100、200、300 条/页。合并单元格需要在同一页内选择连续单元格，数据较多时可切换到 200 或 300 条/页。
...
执行结果下拉框不受编辑模式开关限制。选择“阻塞”或“跳过”时，可以在备注中记录原因；当平台满足总结条件时，顶部“总结结果”会按标题合并后的逻辑用例汇总总用例、已执行、通过、失败、阻塞、跳过，并列出阻塞/跳过原因。一个用例由 Excel 拆成多行后，合并行只计为一条逻辑用例。
...
![导出 Excel 操作完成](docs/images/real-export.png)
